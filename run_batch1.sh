#!/bin/bash
# Batch 1 (region-MIL) — self-contained on-box orchestration.
# waits for region embedding -> merges shards -> runs the region-MIL grid on GPUs 1,6.
set -u
...
print("merged region cache", emb.shape, "frames", len(idx))
PYEOF
