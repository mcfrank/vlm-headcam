"""S3 — the alignment result across all four encoders.
...
for a, (enc, key, lab, col) in zip(axes.ravel(), ENC):
    ...
    print(f"  NOTE figS3 {key}: unfiltered asymptote {A_:.1f}; aligned {np.round(ya,1)}; "
...
for a, l in zip(axes.ravel(), "ABCD"):
    T.panel(a, l, dx=-0.13)
T.save(fig, "figS3_alignment_encoders")
