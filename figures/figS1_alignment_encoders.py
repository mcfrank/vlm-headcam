"""S1 — the alignment result across all four encoders.
...
for a, (enc, key, lab, col) in zip(axes.ravel(), ENC):
    ...
    print(f"  NOTE figS1 {key}: unfiltered asymptote {A_:.1f}; aligned {np.round(ya,1)}; "
...
for a, l in zip(axes.ravel(), "ABCD"):
    T.panel(a, l, dx=-0.13)
T.save(fig, "figS1_alignment_encoders")
