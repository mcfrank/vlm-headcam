...
pos = d[(d.alignment >= 90) & (d.referent.str.len() > 0) & d.nw.between(4, 10)]
pos = pos[[r.lower() in t.lower() for r, t in zip(pos.referent, pos.text)]]
pos = pos.sample(frac=1, random_state=7).drop_duplicates("referent").drop_duplicates("child_id").head(4 * a.n)
...
neg = neg.sample(frac=1, random_state=7).drop_duplicates("text").drop_duplicates("child_id").head(4 * a.n)
...
pos.assign(k=range(len(pos)))[["k", "video_id", "frame_idx", "alignment", "referent", "text"]].to_csv(out / "pos.csv", index=False)
...
print(pos[["alignment", "referent", "text"]].to_string())
